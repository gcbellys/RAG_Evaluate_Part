...
class RerunWorkflow:
    ...
    def __init__(self, report_id: int, config_path: str = "config/config.yaml", rag_cache_dir: str = None,
                 data_dir: str = None, sim_threshold: float = 0.6, organ_majority_threshold: float = 0.5,
                 max_refs: int = 6, results_namespace: str = None,
                 require_baseline: bool = False, baseline_only: bool = False):
        ...
        results_root = self.project_root / output_base
        # 命名空间：将各类结果隔离到 results/<namespace>/
        if results_namespace:
            results_root = results_root / results_namespace
        self.output_base_dir = results_root
        
        # 运行参数
        self.data_dir = data_dir
        self.sim_threshold = sim_threshold
        self.organ_majority_threshold = organ_majority_threshold
        self.max_refs = max_refs
        self.require_baseline = require_baseline
        self.baseline_only = baseline_only

        # RAG缓存文件的存放位置 - 支持自定义目录
        if rag_cache_dir:
            self.rag_output_dir = self.project_root / rag_cache_dir
        else:
            self.rag_output_dir = self.output_base_dir / "rag_search_output"
        
        # 统一在输出目录下管理所有结果
        self.baseline_results_dir = self.output_base_dir / "baseline_results"
        self.rerun_results_dir = self.output_base_dir / "rerun_with_rag"
        self.comparison_results_dir = self.output_base_dir / "rerun_comparisons"
        ...
        print(f"🛡️ 兜底阈值: top1_sim<{self.sim_threshold} 或 主簇占比<{self.organ_majority_threshold}")

    def find_latest_rag_cache(self) -> Path:
        """根据报告ID查找最新的RAG结果文件 (.jsonl)"""
        # 兼容多种命名格式：原始格式、FIXED格式、uniform格式等
        patterns = [
            f"report_{self.report_id}_ragoutcome_*.jsonl",
            f"report_{self.report_id}_ragoutcome:*.jsonl",
            f"report_{self.report_id}_FIXED_ragoutcome_*.jsonl",
            f"report_{self.report_id}_*ragoutcome*.jsonl",
        ]
        files: List[str] = []
        for pattern in patterns:
            files.extend(glob.glob(str(self.rag_output_dir / pattern)))
    ...
    def find_or_create_baseline_results(self) -> Path:
        ...
        # 优先在全局baseline目录（无命名空间）查找，避免重复生成
        # 兼容两种命名：report_diagnostic_43001_... 与 report_diagnostic_diagnostic_43001_...
        search_patterns = [
            f"report_diagnostic_{self.report_id}_evaluation_*.json",
            f"report_diagnostic_diagnostic_{self.report_id}_evaluation_*.json",
        ]
        global_baseline_dir = self.project_root / self.config.get_path('output_results') / 'baseline_results'
        files = []
        for pattern in search_patterns:
            files.extend(glob.glob(str(global_baseline_dir / pattern)))
            files.extend(glob.glob(str(self.baseline_results_dir / pattern)))
        ...
        old_files = []
        for pattern in search_patterns:
            old_files.extend(glob.glob(str(old_results_dir / pattern)))
        ...
        if self.require_baseline:
            raise FileNotFoundError(f"未找到报告ID {self.report_id} 的baseline结果，且已设置require_baseline")
    ...
    def _run_baseline_evaluation(self) -> Path:
        ...
        # 确保API客户端已初始化（baseline_only 时 run() 会直接调用本函数，需要在此初始化）
        print("🔧 初始化API连接(基线)...")
        try:
            if not self.api_manager.initialize_clients(self.config.config):
                raise Exception("API客户端初始化失败(基线)")
        except Exception as e:
            raise Exception(f"API初始化失败(基线): {e}")
        ...
        # 寻找测试文件（优先使用传入的数据目录）
        if self.data_dir:
            test_data_path = Path(self.data_dir)
        else:
            ...
            try:
                ...
                for api_name, response in api_result.items():
                    api_response_data = {
                        ...
                        'organ_names': response.get('organ_names', []),  # 🔧 修复：支持多器官
                        'total_organs': response.get('total_organs', 0),  # 🔧 修复：器官总数
    ...
    def _evaluate_rag_quality(self, rag_results: Any) -> Tuple[float, str]:
        """评估RAG结果的质量和可靠性，加入信息一致性检查（兼容 list/dict）"""
        if not rag_results:
            return 0.0, "无检索结果"

        entries = self._extract_rag_entries(rag_results)
        if not entries:
            return 0.0, "无有效结果"
        ...
        for e in entries:
            total_refs += 1
            text = e.get('text', '') or ''
            organ_name = e.get('organName', '') or ''
            locs = e.get('anatomicalLocations', []) or []
            ...
            if organ_name:
                ...
                if locs:
                    quality_score += 0.2
            if any(m in text.lower() for m in ['diagnosis', 'condition', 'disease', 'syndrome', 'disorder']):
                quality_score += 0.1

        avg_quality = quality_score / max(total_refs, 1)
        ...
        if consistency_penalty > 0:
            quality_indicators.append("信息冲突严重" if consistency_penalty >= 0.4 else "信息轻微冲突")

        return min(avg_quality, 1.0), "; ".join(quality_indicators)

    def _extract_rag_entries(self, rag_payload: Any) -> List[Dict[str, Any]]:
        """统一抽取RAG条目，包含 organ/locations/text/similarity/rank（兼容's'和'rag_results'）"""
        entries: List[Dict[str, Any]] = []
        if not rag_payload:
            return entries
        if isinstance(rag_payload, dict):
            # 旧's'结构
            for _, value in rag_payload.items():
                if isinstance(value, dict) and 'units' in value:
                    for unit in value.get('units', []):
                        u = unit.get('u_unit', {})
                        organ = (u.get('o_organ', {}) or {})
                        entries.append({
                            'text': u.get('d_diagnosis', ''),
                            'organName': organ.get('organName', ''),
                            'anatomicalLocations': organ.get('anatomicalLocations', []) or [],
                            'similarity': None,
                            'rank': None,
                            'brief_reason': (u.get('b_textual_basis', {}) or {}).get('medicalInference', '')
                        })
        elif isinstance(rag_payload, list):
            # 新'rag_results'列表
            for item in rag_payload:
                sim = item.get('cosine_similarity')
                rank = item.get('rank')
                metadata = item.get('metadata', {})
                for uu in metadata.get('U_unit_set', []):
                    u = uu.get('u_unit', {})
                    organ = (u.get('o_organ', {}) or {})
                    entries.append({
                        'text': u.get('d_diagnosis', ''),
    ...
    def _should_disable_rag(self, rag_payload: Any) -> Tuple[bool, str]:
        """基于top1相似度与器官一致性（主簇占比）判断是否禁用RAG"""
        entries = self._extract_rag_entries(rag_payload)
        if not entries:
            return True, "无RAG条目"
        sims = [e['similarity'] for e in entries if isinstance(e.get('similarity'), (int, float))]
        top1 = max(sims) if sims else None
        organ_counts: Dict[str, int] = {}
        for e in entries:
            name = e.get('organName') or ''
            if name:
                organ_counts[name] = organ_counts.get(name, 0) + 1
        if organ_counts:
            total = sum(organ_counts.values())
            major = max(organ_counts.values())
            major_ratio = major / max(total, 1)
        ...
        reasons = []
        disable = False
        if top1 is not None and top1 < self.sim_threshold:
            disable = True
            reasons.append(f"top1_sim<{self.sim_threshold} ({top1:.3f})")
        if unique_organs > 1 and major_ratio < self.organ_majority_threshold:
            disable = True
            reasons.append(f"器官多簇且主簇占比<{self.organ_majority_threshold} ({major_ratio:.2f})")
        return disable, "; ".join(reasons) if reasons else "通过"

    def _purify_rag_context(self, rag_payload: Any) -> List[Dict[str, Any]]:
        """🔧 修复：支持多器官RAG上下文，不再限制为单一器官簇"""
        entries = self._extract_rag_entries(rag_payload)
        if not entries:
            return []
        ...
        for e in entries:
            key = e.get('organName') or 'UNKNOWN'
            clusters.setdefault(key, []).append(e)
        
        # 🔧 修复：不再只选择主要器官簇，而是按重要性排序所有器官
        sorted_organs = sorted(clusters.keys(), key=lambda k: len(clusters[k]), reverse=True)
        
        seen = set()
        purified: List[Dict[str, Any]] = []
        
        # 🔧 修复：从所有器官簇中选择高质量条目，而不是只从主簇
        for organ_name in sorted_organs:
            organ_entries = clusters[organ_name]
            
            # 排序优先高相似度和靠前rank
            for e in sorted(organ_entries, key=lambda x: (-(x.get('similarity') or 0), x.get('rank') or 9999)):
                sig = (e.get('text', ''), tuple(sorted(e.get('anatomicalLocations', []))))
                if sig in seen:
                    continue
                seen.add(sig)
                ...
                purified.append({
                    'organName': e.get('organName', ''),
                    'anatomicalLocations': anatomical_locations
                })
    ...
    def _build_augmented_prompt(self, original_query: str, rag_results: Dict[str, Any]) -> str:
        ...
        """
        # 兜底判定：低质或冲突严重直接禁用RAG
        disable_rag, reason = self._should_disable_rag(rag_results)
        if disable_rag:
            aug_parts = [
                "🔍 症状分析任务：",
                f"症状：{original_query.strip()}",
                "",
                "📚 检索系统未提供可靠的参考信息（已禁用RAG）",
                f"原因：{reason if reason else '低置信度或信息冲突'}",
                "",
                "请严格按照JSON格式输出：",
                "{",
                '  "organ": "主要相关器官",',
        ...
        purified_refs = self._purify_rag_context(rag_results)
        ...
        # 根据RAG质量调整策略
        if purified_refs and rag_quality > 0.0:
            primary_block = json.dumps(purified_refs, ensure_ascii=False, indent=2)
            ...
            aug_parts.extend([
                ...
                primary_block,
                "",
                "请首先基于您的医学专业知识独立给出判断；若参考信息与您的常识一致且有助于结论，再择要吸收。",
                "",
                "请严格按照JSON格式输出：",
                "{",
                '  "organ": "主要相关器官",',
                ...
                '  "decision_rationale": "独立判断为主，必要时参考RAG且说明取舍"',
                "}"
            ])
        else:
    ...
    def run(self):
        """执行完整的工作流程"""
        try:
            # baseline-only 模式：只生成baseline后退出
            ...
            # 加载RAG增强提示词（优先），不存在则回退到baseline系统提示词
            rag_prompt_path = Path("prompt/rag_enhanced_prompt.txt")
            system_prompt_path = Path("prompt/system_prompt.txt")
            ...
            with open(rag_cache_file, 'r', encoding='utf-8') as f:
                for i, line in enumerate(f):
                    try:
                        ...
                        # 兼容两种缓存结构（旧's'与新'rag_results'）
                        rag_s_block = data.get("s") or data.get("rag_results", {})
    ...
    def _compare_responses(self, baseline_responses: Dict[str, Any], rag_responses: Dict[str, Any], expected_results: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for api_name in common_apis:
            ...
            # 保护策略：若baseline器官已精确匹配，冻结器官且限制RAG位置信息不超过baseline规模
            if baseline_organ_accuracy.get('category') == 'exact_match':
                # 冻结器官：沿用baseline_organ
                rag_organ = baseline_organ
                # 限制RAG位置信息规模（不超过baseline的长度）
                if len(rag_locations) > len(baseline_locations):
                    rag_locations = rag_locations[:len(baseline_locations)]
    ...
    def _assess_overall_improvement(self, baseline_metrics: Dict[str, float], rag_metrics: Dict[str, float], 
        ...
        elif rag_metrics['f1_score'] == baseline_metrics['f1_score']:
            return "⚪ RAG增强无明显影响"
        else:
    ...
    def _generate_score_report(self, simplified_results: Dict[str, Any], result_folder: Path, timestamp: str) -> Path:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for api_name in api_names:
                stats = api_stats[api_name]
                total_symptoms = len(symptom_details)
                f.write(f"\n【{api_name.upper()}】\n")
            ...
            f.write("█ 平均指标改善\n")
            f.write("-" * 60 + "\n")
            f.write(f"{'API':<12} {'精确率':<10} {'召回率':<10} {'F1分数':<10}\n")
            f.write("-" * 60 + "\n")
            for api_name in api_names:
                stats = api_stats[api_name]
            ...
            # 找出表现最好和最差的API（基于平均F1改善）
            best_api = max(api_names, key=lambda x: api_stats[x]['avg_f1']) if api_names else ''
            worst_api = min(api_names, key=lambda x: api_stats[x]['avg_f1']) if api_names else ''
            
            f.write(f"\n【最佳表现API】: {best_api.upper()}\n")
            f.write(f"  平均F1改善: {api_stats[best_api]['avg_f1']:+.1f}%\n")
            f.write(f"  改善症状比例: {api_stats[best_api]['positive_effects']/len(symptom_details)*100:.1f}%\n")
            
            f.write(f"\n【需要改进API】: {worst_api.upper()}\n")
            f.write(f"  平均F1改善: {api_stats[worst_api]['avg_f1']:+.1f}%\n")
            f.write(f"  负面影响症状比例: {api_stats[worst_api]['negative_effects']/len(symptom_details)*100:.1f}%\n")
            ...
            total_evaluations = len(symptom_details) * len(api_names) if api_names else 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rag_cache_dir", help="RAG缓存目录路径 (相对于项目根目录)")
    parser.add_argument("--data_dir", help="测试数据目录 (包含 diagnostic_*.json)")
    parser.add_argument("--sim_threshold", type=float, default=0.5, help="RAG禁用的top-1相似度阈值 (降低以减少禁用)")
    parser.add_argument("--organ_majority_threshold", type=float, default=0.4, help="器官主簇最小占比阈值 (降低以支持多器官)")
    parser.add_argument("--max_refs", type=int, default=6, help="传递给LLM的RAG参考条目上限 (增加以提供更多信息)")
    parser.add_argument("--results_namespace", help="结果命名空间（子目录），用于多数据库隔离输出")
    parser.add_argument("--require_baseline", action="store_true", help="要求baseline已存在，缺失则报错")
    parser.add_argument("--baseline_only", action="store_true", help="仅生成baseline并退出，不做RAG评测")
    ...
    workflow = RerunWorkflow(
        ...
        organ_majority_threshold=args.organ_majority_threshold,
