...
# -*- coding: utf-8 -*-
"""
Uniform 数据库 - 新工作流（不改动现有代码）
顺序：Baseline → 新RAG检索（confidence-aware）→ RAG增强评估（使用新prompt）
- 基线评估：workflows/main_workflow_batch.py（不含RAG）
- 新RAG检索：scripts/confidence_aware_rag_adapter.py（写入 results/rag_output_uniform）
- RAG增强评估：workflows/rerun_with_rag.py（读取 rag_cache_dir 并对比 baseline）
- Prompt策略：暂时性将 prompt/system_prompt.txt 替换为 prompt/rag_enhanced_prompt.txt，结束后恢复

用法：
  python workflows/uniform_full_workflow_newprompt.py 43001               # 单个报告
  python workflows/uniform_full_workflow_newprompt.py 43001 43005        # 报告范围
  python workflows/uniform_full_workflow_newprompt.py 43001 43005 --top_k 3 --config config/config_cn.yaml
"""
...
def main():
    parser = argparse.ArgumentParser(description="Uniform新工作流：Baseline→RAG（新检索+新prompt）")
    ...
    print("=" * 80)
    print("🎯 Uniform新工作流：Baseline → 新RAG → RAG评估")
    print("=" * 80)
    ...
    if not replace_system_prompt_temporarily():
        print("⚠️ 无法启用增强版prompt，继续使用现有system_prompt")

    try:
        # 步骤1：Baseline（批处理优化版）
        print("\n" + "-" * 60)
        print("📊 步骤1/3：运行Baseline评估（不含RAG）")
        baseline_cmd = f"python {BASE_DIR}/workflows/main_workflow_batch.py --start_id {start_id} --end_id {end_id} --config {args.config}"
        if args.data_dir != "test_set":
            baseline_cmd += f" --data-dir {args.data_dir}"
        if run_cmd(baseline_cmd, "Baseline评估"): 
            success_steps += 1
        else:
            print("❌ Baseline评估失败，退出")
            return

        # 步骤2：新RAG检索（confidence-aware，写入统一缓存）
        print("\n" + "-" * 60)
        print("🔎 步骤2/3：运行新RAG检索（confidence-aware）")
        retrieval_ok = True
        for report_id in range(start_id, end_id + 1):
            ...
            rag_cmd = (
                ...
                f"--output_dir results/rag_output_uniform --rag_db_type uniform"
            )
        ...
        # 步骤3：RAG增强评估（读取统一缓存，使用新prompt）
        print("\n" + "-" * 60)
        print("🚀 步骤3/3：运行RAG增强评估（对比Baseline）")
        rerun_ok = True
        for report_id in range(start_id, end_id + 1):
            rerun_cmd = (
                f"python {BASE_DIR}/workflows/rerun_with_rag.py {report_id} "
                f"--config {args.config} --rag_cache_dir results/rag_output_uniform"
            )
            if args.data_dir != "test_set":
