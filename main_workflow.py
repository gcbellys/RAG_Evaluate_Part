...
class MainWorkflow:
    ...
    def _standardize_results(self, report_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for symptom_idx, symptom in enumerate(report_results['symptoms']):
            # 构建期望结果
            expected_result = {}
            if symptom.get('expected_organs'):
                # 合并所有期望器官的信息
                all_organs = []
                all_locations = []
                all_diagnoses = []
                ...
                for expected in symptom['expected_organs']:
                    if isinstance(expected, dict):
                        if 'organName' in expected:
                            all_organs.append(expected['organName'])
                        if 'anatomicalLocations' in expected:
                            all_locations.extend(expected.get('anatomicalLocations', []))
                        if 'd_diagnosis' in expected:
                            all_diagnoses.append(expected['d_diagnosis'])
                    elif isinstance(expected, str):
                        all_organs.append(expected)
                
                expected_result = {
                    'diag': '; '.join(set(all_diagnoses)) if all_diagnoses else symptom.get('diagnosis', ''),
                    'organ': ', '.join(set(all_organs)) if all_organs else '',
                    'a_position': ', '.join(set(all_locations)) if all_locations else ''
                }
            else:
                expected_result = {
